...
public class ClienteDAO {
    ...
    public Cliente getCliente(Cliente cliente) {
        ...
        try {
            ...
            String sql = "SELECT id_cliente idcli, initcap(nome_cliente) nome, cpf_cliente,rg_cliente,dtcad_cliente from tab_cliente where cpf_cliente in ? or nome_cliente in ?";
            ...
            if (rs.next()) {
                ...
                cliente.setDtCad(rs.getDate("dtcad_cliente"));
    ...
    public ArrayList<Cliente> pesquisarCliente(String cpf, String nome) {
        ...
        try {
            Connection con = co.abreConexao();
            String sql = "SELECT id_cliente, id_logradouro,initcap(nome_cliente)nome,cpf_cliente cpf,rg_cliente rg, dtnasc_cliente nasc,initcap(sexo_cliente)sexo,"
    ...
    public String atualizaCliente(Cliente cliente) {
        ...
        try {
            Connection con = co.abreConexao();
            CallableStatement cs = con.prepareCall("{call altera_cliente(?,?,?,?,?,?,?,?,?,?,?,?,?)}");
            cs.setInt(1, cliente.getIdCliente());
            cs.setString(2, cliente.getNomeCliente());
            cs.setString(3, cliente.getCpf());
            cs.setString(4, cliente.getRg());
            cs.setDate(5, (Date) cliente.getDtNasc());
            cs.setString(6, cliente.getSexo());
            cs.setString(7, cliente.getEndereco().getNumero());
            cs.setString(8, cliente.getEndereco().getComplemento());
            cs.setInt(9, cliente.getEndereco().getIdLogradouro());
            cs.setString(10, cliente.getContato().getEmail());
            cs.setString(11, cliente.getContato().getTelefone());
            cs.setString(12, cliente.getContato().getCelular());
            cs.setString(13, cliente.getContato().getObs());
            cs.execute();
            cs.close();
            con.close();
            msg = "Proprietário Alterado com Sucesso!";
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(null, "Erro: " + ex);
        }
        return msg;
